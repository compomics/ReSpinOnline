...
// ZOOMING
    ...
    function resetAxisZoom(container) {
        ...
        if (zoom_x && zoom_y) {
            ...
            if (plot)
                plot.getOptions().selection.mode = "x";
        } else if (zoom_y) {
            plotOptions.selection.mode = "y";
            if (plot)
                plot.getOptions().selection.mode = "y";
        }
    }
    ...
    function calculateTheoreticalSeries(container, selectedIons) {
        ...
        if (selectedIons) {
            ...
            for (var i = 0; i < selectedIons.length; i += 1) {
                ...
                if (sion.type == "a") {
                    if (!container.data("massTypeChanged") && ionSeries.a[sion.charge])
                ...
                if (sion.type == "b") {
                    if (!container.data("massTypeChanged") && ionSeries.b[sion.charge])
                        continue; // already calculated
                    else {
                        todoIonSeries.push(sion);
                ...
                if (sion.type == "c") {
                    if (!container.data("massTypeChanged") && ionSeries.c[sion.charge])
                        continue; // already calculated
                    else {
                        todoIonSeries.push(sion);
                ...
                if (sion.type == "x") {
                    if (!container.data("massTypeChanged") && ionSeries.x[sion.charge])
                        continue; // already calculated
                    else {
                        todoIonSeries.push(sion);
                ...
                if (sion.type == "y") {
                    if (!container.data("massTypeChanged") && ionSeries.y[sion.charge])
                        continue; // already calculated
                    else {
                        todoIonSeries.push(sion);
                ...
                if (sion.type == "z") {
                    if (!container.data("massTypeChanged") && ionSeries.z[sion.charge])
                        continue; // already calculated
                    else {
                        todoIonSeries.push(sion);
                ...
                if (sion.type == "mh") {
                    if (!container.data("massTypeChanged") && ionSeries.mh[sion.charge])
                        continue; // already calculated
                    else {
                        todoIonSeries.push(sion);
...
// -----------------------------------------------
    ...
    function initContainer(container) {
        ...
        var options = container.data("options");
        var rowspan = 2;
        var parentTable = '<table cellpadding="0" cellspacing="5" style="width:100%"> ';
        parentTable += '<tbody> ';
        parentTable += '<tr> ';
        // Header
...
//---------------------------------------------------------
    function showSequenceInfo(container) {
        ...
        var specinfo = '';
        if (options.sequence) {
            specinfo += '<div style="width:100%">';
            specinfo += '<h1>ReSpin PSM Summary</h3><b1>';
            specinfo += '<table style="width:100%">';
            specinfo += '<tr><td>Peptide</td><td>' + getModifiedSequence(options) + '</td></tr>';
            var neutralMass = 0;
